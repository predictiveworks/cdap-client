...
/*
 * Changed to project internal metrics to reduce dependencies
 * Dr. Krusche & Partner PartG
 */
import co.cask.cdap.client.common.metrics.MetricsTags;
import co.cask.cdap.client.proto.MetricTagValue;
import co.cask.cdap.client.proto.TimeValue;
import co.cask.cdap.client.proto.MetricQueryResult;
...
public class MetricsClient {
  ...
  public List<MetricTagValue> searchTags(Map<String, String> tags)
	 ...
	 */
    List<Map<String,Object>> result = ObjectResponse.fromJsonBody(
    	      response, new TypeToken<List<Map<String,Object>>>() {
    	  		private static final long serialVersionUID = -822824745018692816L; }).getResponseObject();
    
    List<MetricTagValue> values = new ArrayList<MetricTagValue>();
	for (Map<String,Object> value: result) {
		values.add(new MetricTagValue(value));
	}
  ...
  public List<String> searchMetrics(Map<String, String> tags)
    ...
    ObjectResponse<List<String>> result = ObjectResponse.fromJsonBody(
      response, new TypeToken<List<String>>() {
		private static final long serialVersionUID = 7839756103258684601L; }.getType());
  ...
  public MetricQueryResult query(Map<String, String> tags, List<String> metrics, List<String> groupBys,
	 ...
	 * implementation has an issue with Gson
	 */
    Map<String,Object> result = ObjectResponse.fromJsonBody(
    	      response, new TypeToken<Map<String,Object>>() {
				private static final long serialVersionUID = -3528786927520768898L;
    	  		}).getResponseObject();
    
    return new MetricQueryResult(result);
  }
  ...
  private long getTotalCounter(Map<String, String> tags, String metricName) {
    try {
      ...
      if (result.getSeries().size() == 0) {
        return 0;
      }

      List<TimeValue> timeValues = result.getSeries().get(0).getData();
