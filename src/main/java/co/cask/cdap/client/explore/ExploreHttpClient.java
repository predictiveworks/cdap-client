...
package co.cask.cdap.client.explore;
...
import co.cask.cdap.client.common.ServiceUnavailableException;
import co.cask.cdap.client.common.UnauthenticatedException;
import co.cask.cdap.client.common.conf.Constants;
import co.cask.cdap.client.common.http.DefaultHttpRequestConfig;
import co.cask.cdap.client.explore.service.Explore;
import co.cask.cdap.client.explore.service.ExploreException;
import co.cask.cdap.client.explore.service.HandleNotFoundException;
import co.cask.cdap.client.explore.service.MetaDataInfo;
import co.cask.cdap.client.explore.service.TableNotFoundException;
import co.cask.cdap.client.explore.utils.ColumnsArgs;
import co.cask.cdap.client.explore.utils.FunctionsArgs;
import co.cask.cdap.client.explore.utils.SchemasArgs;
import co.cask.cdap.client.explore.utils.TablesArgs;
import co.cask.cdap.internal.io.SchemaTypeAdapter;
import co.cask.cdap.proto.NamespaceMeta;
import co.cask.cdap.proto.QueryHandle;
import co.cask.cdap.proto.QueryInfo;
import co.cask.cdap.client.proto.ColumnDesc;
import co.cask.cdap.client.proto.QueryResult;
import co.cask.cdap.client.proto.QueryStatus;
...
/**
 * The methods of this class call the HTTP APIs exposed by explore and return
 * the raw information contained in their json responses. This class is only
 * meant to be extended by classes which implement ExploreClient.
 */
abstract class ExploreHttpClient implements Explore {
	...
	private static final Type MAP_TYPE_TOKEN = new TypeToken<Map<String, String>>() {
	}.getType();
	private static final Type TABLES_TYPE = new TypeToken<List<TableNameInfo>>() {
	}.getType();
	private static final Type QUERY_INFO_LIST_TYPE = new TypeToken<List<QueryInfo>>() {
	}.getType();
	...
	protected String getUserId() {
		// by default, return null, it is only required to be set by
		// DiscoveryExploreClient
	...
	protected Map<String, String> addAdditionalSecurityHeaders() {
		// by default return null. It is only required to set addition security headers
		// if needed as in case of
	...
	public void ping() throws UnauthenticatedException, ServiceUnavailableException, ExploreException {
		...
		throw new ExploreException(String.format(
				"Unexpected response while checking explore status. " + "Received code '%s' and response message '%s'.",
				response.getResponseCode(), response.getResponseBodyAsString()));
	}

	protected QueryHandle doEnableExploreStream(StreamId stream, String tableName, FormatSpecification format)
			throws ExploreException {
		HttpResponse response = doPost(String.format("namespaces/%s/data/explore/streams/%s/tables/%s/enable",
				stream.getNamespace(), stream.getEntityName(), tableName), format == null ? null : GSON.toJson(format),
				null);
		if (response.getResponseCode() == HttpURLConnection.HTTP_OK) {
	...
	protected QueryHandle doAddPartition(DatasetId datasetInstance, DatasetSpecification spec, PartitionKey key,
			String path) throws ExploreException {
	...
	private QueryHandle doPartitionOperation(DatasetId datasetId, DatasetSpecification spec, PartitionKey key,
		...
		return doPartitionOperation(datasetId, spec, key, endpoint, operationName,
				Collections.<String, String>emptyMap());
	}

	private QueryHandle doPartitionOperation(DatasetId datasetId, DatasetSpecification spec, PartitionKey key,
			String endpoint, String operationName, Map<String, String> additionalArguments) throws ExploreException {
		...
		HttpResponse response = doPost(String.format("namespaces/%s/data/explore/datasets/%s/%s",
				datasetId.getNamespace(), datasetId.getEntityName(), endpoint), GSON.toJson(args), null);
		if (response.getResponseCode() == HttpURLConnection.HTTP_OK) {
			return QueryHandle.fromId(parseResponseAsMap(response, "handle"));
		}
	...
	protected QueryHandle doUpdateExploreDataset(DatasetId datasetInstance, DatasetSpecification oldSpec,
			DatasetSpecification newSpec) throws ExploreException {
		HttpResponse response = doPost(
				String.format("namespaces/%s/data/explore/datasets/%s/update", datasetInstance.getNamespace(),
		...
		throw new ExploreException(
				String.format("Cannot update explore on dataset %s. Reason: %s", datasetInstance.toString(), response));
	}

	protected QueryHandle doEnableExploreDataset(DatasetId datasetInstance, DatasetSpecification spec,
		...
		HttpResponse response = doPost(String.format("namespaces/%s/data/explore/datasets/%s/%s",
				datasetInstance.getNamespace(), datasetInstance.getEntityName(), endpoint), body, null);
		if (response.getResponseCode() == HttpURLConnection.HTTP_OK) {
			return QueryHandle.fromId(parseResponseAsMap(response, "handle"));
		}
		throw new ExploreException(
				String.format("Cannot enable explore on dataset %s. Reason: %s", datasetInstance.toString(), response));
	}

	protected QueryHandle doDisableExploreDataset(DatasetId datasetInstance, DatasetSpecification spec)
			throws ExploreException {
	...
	public QueryHandle execute(NamespaceId namespace, String statement,
		...
		Map<String, String> bodyMap = additionalSessionConf == null ? ImmutableMap.of("query", statement)
	...
	public QueryStatus getStatus(QueryHandle handle) throws ExploreException, HandleNotFoundException {
		...
		if (response.getResponseCode() == HttpURLConnection.HTTP_OK) {
			 ...
			 * with Gson
			 */
			Type typeOfObject = new TypeToken<Map<String, Object>>() {
			}.getType();
			Map<String, Object> status = new Gson().fromJson(new String(response.getResponseBody(), Charsets.UTF_8),
					typeOfObject);
	...
	public List<ColumnDesc> getResultSchema(QueryHandle handle) throws ExploreException, HandleNotFoundException {
		...
		if (response.getResponseCode() == HttpURLConnection.HTTP_OK) {
			 ...
			 * with Gson
			 */
			Type typeOfObject = new TypeToken<List<Map<String, Object>>>() {
			}.getType();

			List<Map<String, Object>> records = new Gson()
					.fromJson(new String(response.getResponseBody(), Charsets.UTF_8), typeOfObject);
			List<ColumnDesc> columns = new ArrayList<ColumnDesc>();
			for (Map<String, Object> column : records) {
				columns.add(new ColumnDesc(column));
			}
	...
	public List<QueryResult> previewResults(QueryHandle handle)
		...
		HttpResponse response = doPost(String.format("data/explore/queries/%s/%s", handle.getHandle(), "preview"), null,
				null);
		if (response.getResponseCode() == HttpURLConnection.HTTP_OK) {
			/*
			 * Extraction of response body changed, as initial implementation has an issue
	...
	@Override
	public QueryHandle getColumns(String catalog, String schemaPattern, String tableNamePattern,
			String columnNamePattern) throws ExploreException, SQLException {
	...
	@Override
	public QueryHandle getFunctions(@Nullable String catalog, @Nullable String schemaPattern,
	...
	@Override
	public TableInfo getTableInfo(String namespace, String table) throws ExploreException, TableNotFoundException {
	...
	private HttpResponse doRequest(String resource, HttpMethod requestMethod, @Nullable Map<String, String> headers,
		...
		try {
			...
			throw new ExploreException(String.format(
					"Error connecting to Explore Service at %s while doing %s with headers %s and body %s", resolvedUrl,
					requestMethod,
	...
	private HttpRequestConfig createRequestConfig() {
		return new HttpRequestConfig(getHttpRequestConfig().getConnectTimeout(),
				getHttpRequestConfig().getReadTimeout(), verifySSLCert());
	}

	private String resolve(String resource) {
		InetSocketAddress addr = getExploreServiceAddress();
		String url = String.format("%s://%s:%s%s/%s", isSSLEnabled() ? "https" : "http", addr.getHostName(),
				addr.getPort(), Constants.Gateway.API_VERSION_3, resource);
