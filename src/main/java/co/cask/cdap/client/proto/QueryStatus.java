...
public class QueryStatus {
  ...
  public QueryStatus(Map<String,Object> record) {
	  
	  this.hasResults = false;
	  if (record.containsKey("hasResults"))
		  this.hasResults = (Boolean)record.get("hasResults");
	  
	  this.errorMessage = null;
	  if (record.containsKey("errorMessage"))
		  this.errorMessage = (String)record.get("errorMessage");
	  
	  this.sqlState = null;
	  if (record.containsKey("sqlState"))
		  this.sqlState = (String)record.get("sqlState");

	  this.status = OpStatus.ERROR;
	  if (record.containsKey("status")) {
		  String status = (String)record.get("status");
		  this.status = OpStatus.valueOf(status);
	  }
