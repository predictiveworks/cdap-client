...
import co.cask.cdap.client.common.ProgramNotFoundException;
import co.cask.cdap.client.common.UnauthenticatedException;
/*
 * Changed to project internal constants to reduce dependencies
 * Dr. Krusche & Partner PartG
 */
import co.cask.cdap.client.common.conf.Constants;
/*
 * Changed to project internal io to reduce dependencies
 * Dr. Krusche & Partner PartG
 */
import co.cask.cdap.client.common.io.CaseInsensitiveEnumTypeAdapterFactory;
/*
 * Changed to project internal tasks to reduce dependencies
 * Dr. Krusche & Partner PartG
 */
...
import co.cask.cdap.client.proto.RunRecord;
...
public class ProgramClient {
	...
	private static final Type BATCH_STATUS_RESPONSE_TYPE = new TypeToken<List<BatchProgramStatus>>() {
		private static final long serialVersionUID = 4298914428026826823L;
	}.getType();

	private static final Type BATCH_RESULTS_TYPE = new TypeToken<List<BatchProgramResult>>() {
		private static final long serialVersionUID = 383144328720711893L;
	}.getType();

	private static final Type MAP_STRING_STRING_TYPE = new TypeToken<Map<String, String>>() {
		private static final long serialVersionUID = 3939920608142408134L;
	}.getType();
	 ...
	 * @param debug
	 *            true to start in debug mode
	 * @param runtimeArgs
	 *            runtime arguments to pass to the program
	 * @throws IOException
	 ...
	 * @throws ProgramNotFoundException
	 *             if the program with the specified name could not be found
	 * @throws UnauthenticatedException
	 ...
	 *
	 * @param namespace
	 *            the namespace of the programs
	 * @param programs
	 *            the programs to start, including any runtime arguments to start
	 *            them with
	...
	public List<BatchProgramResult> start(NamespaceId namespace, List<BatchProgramStart> programs)
		...
		HttpRequest request = HttpRequest.builder(HttpMethod.POST, url).withBody(GSON.toJson(programs), Charsets.UTF_8)
				.build();
	 ...
	 *             if a network error occurred
	 * @throws ProgramNotFoundException
	 *             if the program with specified name could not be found
	 * @throws UnauthenticatedException
	 *             if the request is not authorized successfully in the gateway
	 *             server
	 */
	 ...
	 */
	public void stopAll(NamespaceId namespace) throws IOException, UnauthenticatedException, InterruptedException,
			TimeoutException, UnauthorizedException, ApplicationNotFoundException {
				...
				try {
					...
					if (!status.equals("STOPPED")) {
						try {
							...
							// ProgramClient#stop calls RestClient, which throws an IOException if the HTTP
							// response code is 400,
							// which can be due to the program already being stopped when calling stop on
							// it.
							// Most likely, there was a race condition that the program stopped between the
							// time we checked its
	...
	public String getStatus(ProgramId programId)
		...
		Map<String, String> responseObject = ObjectResponse
				.<Map<String, String>>fromJsonBody(response, MAP_STRING_STRING_TYPE, GSON).getResponseObject();
	 ...
	 *            the namespace of the programs
	 * @param programs
	 *            the list of programs to get status for
	 ...
	 *            the desired status
	 * @param timeout
	 *            how long to wait in milliseconds until timing out
	 * @throws IOException
	 *             if a network error occurred
	 * @throws ProgramNotFoundException
	 *             if the program with the specified name could not be found
	 ...
	 *             if the program did not achieve the desired program status before
	 *             the timeout
	 * @throws InterruptedException
	 *             if interrupted while waiting for the desired program status
	 */
	public void waitForStatus(final ProgramId program, ProgramStatus status, long timeout, TimeUnit timeoutUnit)
			throws UnauthenticatedException, IOException, ProgramNotFoundException, TimeoutException,
	...
	/**
	 * Gets the live information of a program. In distributed CDAP, this will
	 * contain IDs of the YARN applications that are running the program.
	 *
	 * @param program
	 *            the program
	...
	public DistributedProgramLiveInfo getLiveInfo(ProgramId program)
		...
		String path = String.format("apps/%s/%s/%s/live-info", program.getApplication(),
	 ...
	 *             if the application, flow, or flowlet could not be found
	 * @throws UnauthenticatedException
	 *             if the request is not authorized successfully in the gateway
	 *             server
	 */
	public int getFlowletInstances(FlowletId flowlet)
		...
		URL url = config.resolveNamespacedURLV3(flowlet.getParent().getNamespaceId(),
				String.format("apps/%s/flows/%s/flowlets/%s/instances", flowlet.getApplication(), flowlet.getFlow(),
						flowlet.getFlowlet()));
		HttpResponse response = restClient.execute(HttpMethod.GET, url, config.getAccessToken(),
				HttpURLConnection.HTTP_NOT_FOUND);
		if (response.getResponseCode() == HttpURLConnection.HTTP_NOT_FOUND) {
	 ...
	 *            the flowlet
	 * @param instances
	 *            number of instances for the flowlet to run on
	 * @throws IOException
	 *             if a network error occurred
	 * @throws NotFoundException
	 *             if the application, flow, or flowlet could not be found
	 ...
	 *             if a network error occurred
	 * @throws NotFoundException
	 *             if the application or worker could not be found
	 * @throws UnauthenticatedException
	 *             if the request is not authorized successfully in the gateway
	 *             server
	 */
	public int getWorkerInstances(ProgramId worker)
		...
		URL url = config.resolveNamespacedURLV3(worker.getNamespaceId(),
				String.format("apps/%s/workers/%s/instances", worker.getApplication(), worker.getProgram()));
		HttpResponse response = restClient.execute(HttpMethod.GET, url, config.getAccessToken(),
				HttpURLConnection.HTTP_NOT_FOUND);
		if (response.getResponseCode() == HttpURLConnection.HTTP_NOT_FOUND) {
	 ...
	 *
	 * @param instances
	 *            number of instances for the worker to run on
	 * @throws IOException
	 *             if a network error occurred
	 * @throws NotFoundException
	 *             if the application or worker could not be found
	 ...
	 *             if a network error occurred
	 * @throws NotFoundException
	 *             if the application or service could not found
	 * @throws UnauthenticatedException
	 *             if the request is not authorized successfully in the gateway
	 *             server
	 */
	public int getServiceInstances(ServiceId service)
		...
		URL url = config.resolveNamespacedURLV3(service.getNamespaceId(),
				String.format("apps/%s/services/%s/instances", service.getApplication(), service.getProgram()));
		HttpResponse response = restClient.execute(HttpMethod.GET, url, config.getAccessToken(),
				HttpURLConnection.HTTP_NOT_FOUND);
		if (response.getResponseCode() == HttpURLConnection.HTTP_NOT_FOUND) {
	 ...
	 *             if a network error occurred
	 * @throws NotFoundException
	 *             if the application or service could not be found
	 * @throws UnauthenticatedException
	 *             if the request is not authorized successfully in the gateway
	 *             server
	 */
	 ...
	 *            the program
	 * @param state
	 *            - filter by status of the program
	 * @return the run records of the program
	 * @throws IOException
	 *             if a network error occurred
	 * @throws NotFoundException
	 *             if the application or program could not be found
	 * @throws UnauthenticatedException
	 *             if the request is not authorized successfully in the gateway
	 *             server
	 */
	public List<RunRecord> getProgramRuns(ProgramId program, String state, long startTime, long endTime, int limit)
		...
		String path = String.format("apps/%s/versions/%s/%s/%s/runs?%s", program.getApplication(), program.getVersion(),
		 ...
		 * with Gson
		 */
		List<Map<String, Object>> runs = ObjectResponse
	 ...
	 *            the program
	 * @param start
	 *            start time of the time range of desired logs
	 * @param stop
	 *            end time of the time range of desired logs
	...
	public String getProgramLogs(ProgramId program, long start, long stop)
		...
		String path = String.format("apps/%s/%s/%s/logs?start=%d&stop=%d&escape=false", program.getApplication(),
				program.getType().getCategoryName(), program.getProgram(), start, stop);
		URL url = config.resolveNamespacedURLV3(program.getNamespaceId(), path);
		HttpResponse response = restClient.execute(HttpMethod.GET, url, config.getAccessToken());
		if (response.getResponseCode() == HttpURLConnection.HTTP_NOT_FOUND) {
			throw new ProgramNotFoundException(program);
	...
	public Map<String, String> getRuntimeArgs(ProgramId program)
		...
		String path = String.format("apps/%s/versions/%s/%s/%s/runtimeargs", program.getApplication(),
		...
		return ObjectResponse.fromJsonBody(response, new TypeToken<Map<String, String>>() {
			private static final long serialVersionUID = 7600926313428182197L;
		}).getResponseObject();
	}
