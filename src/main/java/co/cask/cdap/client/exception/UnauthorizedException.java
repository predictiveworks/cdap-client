...
/**
 * Exception thrown when Authentication is successful, but a {@link Principal}
 * is not authorized to perform an {@link Action} on an {@link EntityId}.
 */
public class UnauthorizedException extends RuntimeException implements HttpErrorStatusProvider {

	private static final long serialVersionUID = 31937804959392020L;

	public UnauthorizedException(Principal principal, Action action, EntityId entityId) {
		super(String.format("Principal '%s' is not authorized to perform action '%s' on entity '%s'", principal, action,
				entityId));
	}

	public UnauthorizedException(Principal principal, Set<Action> actions, EntityId entityId) {
		super(String.format("Principal '%s' is not authorized to perform actions '%s' on entity '%s'", principal,
				actions, entityId));
	}
	...
	public UnauthorizedException(Principal principal, Set<Action> actions, EntityId entityId, boolean needHaveAll) {
		super(String.format("Principal '%s' is not authorized to perform %sactions '%s' on entity '%s'", principal,
				needHaveAll ? "" : "any one of the ", actions, entityId));
	}
