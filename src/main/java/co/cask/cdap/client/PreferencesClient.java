...
public class PreferencesClient {
	...
	public Map<String, String> getInstancePreferences()
		...
		return ObjectResponse.fromJsonBody(response, new TypeToken<Map<String, String>>() {
			private static final long serialVersionUID = 4480789538400895079L;
		}).getResponseObject();
	}
	 ...
	 *            Namespace Id
	 * @param resolved
	 *            Set to True if collapsed/resolved properties are desired
	 * @return map of key-value pairs
	 * @throws IOException
	 *             if a network error occurred
	 * @throws UnauthenticatedException
	 ...
	 *             if the requested namespace is not found
	 */
	public Map<String, String> getNamespacePreferences(NamespaceId namespace, boolean resolved)
		...
		URL url = config
				.resolveURLV3(String.format("namespaces/%s/preferences?resolved=%s", namespace.getNamespace(), res));
		HttpResponse response = restClient.execute(HttpMethod.GET, url, config.getAccessToken(),
	...
	public Map<String, String> getApplicationPreferences(ApplicationId application, boolean resolved)
		...
		URL url = config.resolveNamespacedURLV3(application.getParent(),
				String.format("/apps/%s/preferences?resolved=%s", application.getApplication(), res));
		HttpResponse response = restClient.execute(HttpMethod.GET, url, config.getAccessToken(),
				HttpURLConnection.HTTP_NOT_FOUND);
		if (response.getResponseCode() == HttpURLConnection.HTTP_NOT_FOUND) {
		...
		return ObjectResponse.fromJsonBody(response, new TypeToken<Map<String, String>>() {
			private static final long serialVersionUID = -1880905005588418742L;
		}).getResponseObject();
	}
	 ...
	 *             server
	 * @throws NotFoundException
	 *             if the requested application or namespace is not found
	 */
	 ...
	 *             server
	 * @throws NotFoundException
	 *             if the request application or namespace is not found
	 */
	...
	public Map<String, String> getProgramPreferences(ProgramId program, boolean resolved)
		...
		URL url = config.resolveNamespacedURLV3(program.getNamespaceId(),
				String.format("/apps/%s/%s/%s/preferences?resolved=%s", program.getApplication(),
						program.getType().getCategoryName(), program.getProgram(), res));
		HttpResponse response = restClient.execute(HttpMethod.GET, url, config.getAccessToken(),
				HttpURLConnection.HTTP_NOT_FOUND);
		if (response.getResponseCode() == HttpURLConnection.HTTP_NOT_FOUND) {
	...
	public void setProgramPreferences(ProgramId program, Map<String, String> preferences)
			throws IOException, UnauthenticatedException, ProgramNotFoundException, UnauthorizedException {
		URL url = config.resolveNamespacedURLV3(program.getNamespaceId(), String.format("/apps/%s/%s/%s/preferences",
