...
public class ServiceClient {
	...
	private static final Type MAP_STRING_INTEGER_TYPE = new TypeToken<Map<String, Integer>>() {
		...
		private static final long serialVersionUID = 5265937593193856927L;
	}.getType();
	 ...
	 *             if the app or service could not be found
	 */
	public ServiceSpecification get(ProgramId service)
		...
		URL url = config.resolveNamespacedURLV3(service.getNamespaceId(),
				String.format("apps/%s/versions/%s/services/%s", service.getApplication(), service.getVersion(),
						service.getProgram()));
	...
	public void checkAvailability(ServiceId service) throws IOException, UnauthenticatedException, NotFoundException,
		...
		URL url = config.resolveNamespacedURLV3(service.getNamespaceId(),
				String.format("apps/%s/versions/%s/services/%s/available", service.getApplication(),
	...
	/**
	 * Gets a {@link URL} to call methods for a specific version of a
	 * {@link Service}.
	 *
	 * @param service
	 *            {@link ServiceId} of the service
	 * @return a URL to call methods of the service
	 * @throws NotFoundException
	 * 			@throws NotFoundException if the app or service could not be
	 *             found
	 * @throws IOException
	 *             if a network error occurred
	 * @throws UnauthenticatedException
	...
	public URL getVersionedServiceURL(ServiceId service)
		...
		return config.resolveNamespacedURLV3(service.getNamespaceId(),
				String.format("apps/%s/versions/%s/services/%s/methods/", service.getApplication(),
						service.getVersion(), service.getEntityName()));
	}

	/**
	 * Gets a {@link URL} to call methods for the endpoint of a {@link Service} with
	 * no application version. If multiple versions exist, traffic to the
	 * {@link URL} will be distributed to each version according to routing
	 * strategy. If only one version exists, this version will always be reached
	 * with the {@link URL}.
	 *
	 * @param service
	 *            {@link ServiceId} of the service
	...
	public URL getServiceURL(ServiceId service)
			throws NotFoundException, IOException, UnauthenticatedException, UnauthorizedException {
		return config.resolveNamespacedURLV3(service.getNamespaceId(),
				String.format("apps/%s/services/%s/methods/", service.getApplication(), service.getEntityName()));
	}
	 ...
	 *
	 * @param serviceId
	 *            {@link ServiceId} of the service with the application version part
	 *            ignored
	 * @return a Map of {@link String} application version and non-zero
	 *         {@link Integer} percentage of traffic routed to the version.
	 */
	 ...
	 *            ignored
	 * @param routeConfig
	 *            a Map of {@link String} application version and {@link Integer}
	...
	/**
	 * Calls the non-versioned service endpoint for a given method and get routed to
	 * a specific version by the Router
	 *
	 * @param serviceId
	 *            {@link ServiceId} of the service with the application version part
	 *            ignored
	 * @param methodPath
	 *            the path specifying only the method
	...
	public HttpResponse callServiceMethod(ServiceId serviceId, String methodPath)
		...
		String path = String.format("apps/%s/services/%s/methods/%s", serviceId.getApplication(),
				serviceId.getEntityName(), methodPath);
