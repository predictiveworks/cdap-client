 ...
 */
import co.cask.cdap.client.common.AlreadyExistsException;
import co.cask.cdap.client.common.BadRequestException;
import co.cask.cdap.client.common.DatasetModuleAlreadyExistsException;
import co.cask.cdap.client.common.DatasetModuleCannotBeDeletedException;
import co.cask.cdap.client.common.DatasetModuleNotFoundException;
...
public class DatasetModuleClient {
	...
	public List<DatasetModuleMeta> list(NamespaceId namespace)
		...
		return ObjectResponse.fromJsonBody(restClient.execute(HttpMethod.GET, url, config.getAccessToken()),
				new TypeToken<List<DatasetModuleMeta>>() {
					private static final long serialVersionUID = -8654514153314218021L;
				}).getResponseObject();
	}
	 ...
	 *
	 * @param module
	 *            the new dataset module
	 * @param className
	 *            name of the dataset module class within the moduleJarFile
	 * @param moduleJarFile
	 *            Jar file containing the dataset module class and dependencies
	 * @throws BadRequestException
	 *             if the moduleJarFile does not exist
	 * @throws AlreadyExistsException
	 *             if a dataset module with the same name already exists
	 * @throws IOException
	 *             if a network error occurred
	 */
	 ...
	 *
	 * @param module
	 *            the dataset module to delete
	 * @throws DatasetModuleCannotBeDeletedException
	 *             if the dataset module cannot be deleted, usually due to other
	 *             dataset modules or dataset instances using the dataset module
	 * @throws DatasetModuleNotFoundException
	 *             if the dataset module with the specified name was not found
	 * @throws IOException
	 *             if a network error occurred
	 * @throws UnauthenticatedException
	 *             if the request is not authorized successfully in the gateway
	 *             server
	 */
	public void delete(DatasetModuleId module) throws DatasetModuleCannotBeDeletedException,
			DatasetModuleNotFoundException, IOException, UnauthenticatedException, UnauthorizedException {

		URL url = config.resolveNamespacedURLV3(module.getParent(),
				String.format("data/modules/%s", module.getModule()));
		HttpResponse response = restClient.execute(HttpMethod.DELETE, url, config.getAccessToken(),
				HttpURLConnection.HTTP_CONFLICT, HttpURLConnection.HTTP_NOT_FOUND);
	 ...
	 *
	 * @throws DatasetModuleCannotBeDeletedException
	 *             if one of the dataset modules cannot be deleted, usually due to
	 *             existing dataset instances using the dataset module
	 * @throws IOException
	 *             if a network error occurred
	 * @throws UnauthenticatedException
	 *             if the request is not authorized successfully in the gateway
